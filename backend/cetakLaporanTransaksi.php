...
foreach ($products as $key => $product) {
  ...
  $pdf->Cell(40, 6, $dataTransaksiProduk[$key], 1, 0);
  $pdf->Cell(50, 6, "Rp " . number_format($product['harga']), 1, 0);
  $pdf->Cell(50, 6, "Rp " . number_format($product['harga'] * $dataTransaksiProduk[$key]), 1, 1);

  $jumlahTotal += $product['harga'] * $dataTransaksiProduk[$key];
}
